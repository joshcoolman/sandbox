...
import type { LightingPreset } from './LightingSelector'
import type { FilmStockPreset } from './FilmStockSelector'
...
const FILM_STOCK_PRESETS: FilmStockPreset[] = [
  ...
  { title: 'Kodak Vision3 50D', type: 'Cinema / Daylight', grain: 'Ultra-fine grain, maximum clarity and resolution', color: 'Clean, neutral, pristine color reproduction under daylight, clinical precision', palette: ['#e8e8f0', '#88aacc', '#445566'], prompt: 'shot on Kodak Vision3 50D, ultra-fine cinema grain, pristine clean colors, maximum clarity, daylight balanced' },
  { title: 'Kodak Vision3 250D', type: 'Cinema / Daylight', grain: 'Fine grain, versatile cinema stock', color: 'Vivid saturation, wide dynamic range, bold primaries, natural skin under sun', palette: ['#e88830', '#3388bb', '#e8c848'], prompt: 'shot on Kodak Vision3 250D, vivid cinema saturation, wide dynamic range, bold primaries, fine grain, daylight balanced' },
  { title: 'Kodak 5247', type: 'Cinema / Vintage', grain: 'Moderate grain, classic 1970s-80s cinema texture', color: 'Warm cast, slightly muted, rich midtones, vintage Kodak cinema look', palette: ['#c8a070', '#885838', '#3a3028'], prompt: 'shot on Kodak 5247, vintage 1970s-80s cinema stock, moderate grain, warm muted tones, rich midtones' },
  { title: 'Kodak 5254', type: 'Cinema / Vintage', grain: 'Heavy grain, early 1970s cinema stock', color: 'Deep warm amber, heavy grain structure, low-light capable, classic noir warmth', palette: ['#c88830', '#5a3a1a', '#1a1008'], prompt: 'shot on Kodak 5254, early 1970s cinema stock, heavy grain, deep amber warmth, classic low-light rendering' },
]
...
// Maps lighting preset titles to the closest matching film stock preset
const LIGHTING_TO_FILM_STOCK: Record<string, string> = {
  'Blade Runner': 'Kodak Vision3 500T',
  'The Godfather': 'Kodak 5254',
  'The Matrix': 'Kodak Vision3 500T',
  'Mad Max: Fury Road': 'Kodak Vision3 250D',
  'Apocalypse Now': 'Kodak Ektachrome E100',
  '2001: A Space Odyssey': 'Kodak Vision3 50D',
  'The Shining': 'Kodak 5247',
  'Her': 'Fuji Pro 400H',
  'Alien': 'Kodak 5247',
  'Sin City': 'Ilford Delta 3200',
}

export function ImageToUiContent() {
  const [lastOutput, setLastOutput] = useState<{
    component: string
    value: string
  } | null>(null)
  const [filmStockValue, setFilmStockValue] = useState<string | undefined>(undefined)
      ...
      <div className={styles.pageColumns}>
        <div className={styles.pageComponents}>
          ...
          <LightingSelector
            ...
            onExpand={(p) => {
              const match = LIGHTING_TO_FILM_STOCK[p.title]
              if (match) setFilmStockValue(match)
            }}
            onChange={(p) => {
