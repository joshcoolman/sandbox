...
interface CanvasImage {
  id: string
  src: string // data URL for persistence
  x: number
  y: number
  width: number
  height: number
...
type DragMode = 'pan' | 'move' | 'marquee' | null
...
async function savePersistedState(state: PersistedState) {
  try {
    const db = await openDB()
...
export default function MoodboardPage() {
  ...
  useEffect(() => {
    ...
    saveTimerRef.current = setTimeout(() => {
      savePersistedState({ images, transform })
    }, 500)
  }, [images, transform, loaded])
  ...
  const addImagesFromFiles = useCallback(async (files: FileList | File[], cx: number, cy: number) => {
    ...
    // Load all images first to get dimensions for masonry layout
    ...
    // Masonry layout: max 6 columns, uniform column width, proportional height
    ...
    // Add all at once and auto-select the batch
      ...
      if (!handled) {
        ...
        if (text?.match(/\.(png|jpg|jpeg|gif|webp|svg|bmp)(\?.*)?$/i)) {
          ...
          fetch(text).then(r => r.blob()).then(blob => fileToDataUrl(blob)).then(dataUrl => {
            ...
            // Can't convert to data URL, use URL directly (won't persist)
  ...
  const onPointerDown = useCallback((e: React.PointerEvent) => {
    ...
    // Don't capture pointer when clicking toolbar buttons
    if ((e.target as HTMLElement).closest('.mb-toolbar')) return
  ...
  const onPointerUp = useCallback((e: React.PointerEvent) => {
    ...
    if (d.mode === 'marquee' && d.moved) {
      ...
      // Click on empty canvas = set paste target + deselect
  ...
  useEffect(() => {
    ...
    hotkeys('command+0', e => {
      ...
      if (targets.length === 0) { zoomCenter(MAX_SCALE); return }
      const b = getBounds(targets)
      const r = containerRef.current?.getBoundingClientRect()
      if (!r) return
      setTransform({ x: r.width / 2 - (b.x + b.w / 2), y: r.height / 2 - (b.y + b.h / 2), scale: MAX_SCALE })
    })
  ...
  return (
    <div
      ref={containerRef}
      className={`mb-canvas${spaceHeld ? ' mb-pan-mode' : ''}`}
    ...
    >
      ...
      >
        {images.map(img => (
          <div
            ...
            className={`mb-image${selected.has(img.id) ? ' mb-selected' : ''}`}
      ...
      <div className="mb-toolbar">
        ...
        <span className="mb-zoom">{zoomPct}%</span>
      </div>
