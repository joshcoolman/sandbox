...
Currently configured project: `sandbox` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/sandbox/SETUP.md` to fill in
   `data/sandbox/INFO.md`.
...
`--project-id sandbox` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/sandbox/`.
