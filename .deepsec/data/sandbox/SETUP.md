# Agent setup for `sandbox`

This is a deepsec scanning workspace. Project `sandbox` was just registered
(target: `..`). Setup is incomplete — `data/sandbox/INFO.md`
...
2. **Fill in `data/sandbox/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id sandbox
pnpm deepsec process --project-id sandbox
```
